...
async function generateStrategicReport(articles) {
    const context = articles.map((a)=>`[${a.neighborhood}] ${a.title}: ${a.summary}`).join("\n");
    const prompt = `Eres un Analista de Inteligencia Estratégica. 
  Basado en estas noticias de las últimas 24h en Córdoba, genera un análisis técnico:
  1. Tendencia Criminal: ¿Hay un aumento en algún delito específico?
  2. Puntos Calientes (Hotspots): ¿Qué municipios presentan mayor criticidad?
  3. Recomendación de Seguridad: Acciones preventivas sugeridas.
    ...
    try {
        const response = await deepseek.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "Genera reportes de inteligencia breves y tácticos."
                },
                {
                    role: "user",
                    content: prompt
...
// 1. Función para obtener feeds individuales
async function fetchFeed(url, sourceName) {
    try {
        const feed = await parser.parseURL(url);
        const articles = [];
        // Tomamos los 15 más recientes de cada fuente
...
async function fetchAllNews() {
    ...
    // B. Deduplicación y Pre-procesamiento de municipios
    ...
    // C. CLASIFICACIÓN CON IA (Aquí arreglamos el error de "classified is not defined")
    let finalArticles = deduped;
    if (deduped.length > 0) {
        try {
            console.log(`🧠 Clasificando ${deduped.length} noticias con DeepSeek...`);
            finalArticles = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$classify$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["classifyArticles"])(deduped);
            console.log("🚀 Verificando datos antes de enviar a Supabase:");
            console.log("Ejemplo de municipio:", finalArticles[0].neighborhood);
            console.log("Ejemplo de riesgo:", finalArticles[0].threat_level);
        } catch (err) {
            console.error("❌ Error en clasificación IA, se guardarán datos básicos:", err);
        }
    }
    // D. GUARDADO FINAL EN SUPABASE
    if (finalArticles.length > 0) {
        // Filtramos cualquier artículo que no tenga título para evitar el error de Supabase
        ...
        if (error) {
            console.error("❌ [Supabase] Error en upsert:", error.message);
        } else {
            console.log(`✅ ${toSave.length} noticias guardadas con éxito.`);
        }
    }
    return {
...
// 3. Ejecución inmediata (Solo si se llama directamente)
if (/*TURBOPACK member replacement*/ __turbopack_context__.z.main === module) {
    (async ()=>{
        try {
            ...
            console.log(`Total: ${result.total} noticias`);
            console.log("-----------------------------------------");
        } catch (error) {
...
;
;
const revalidate = 60; // Se actualiza cada 30 minutos
async function Home() {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0kzbf9w._.js.map