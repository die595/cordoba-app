...
;
;
async function scrapeFacebookOrganis() {
    let browser = null;
...
// --- [CAMBIO AQUÍ]: Importar el scraper de Facebook ---
...
async function fetchAllNews() {
    ...
    // --- [CAMBIO AQUÍ]: 2. Recolectar Facebook e integrarlo ---
    try {
        console.log("🕵️ Extrayendo datos de Facebook (Organis Noticias)...");
        const fbPosts = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$facebookScraper$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["scrapeFacebookOrganis"])();
        if (fbPosts && fbPosts.length > 0) {
            const fbArticles = fbPosts.map((text, index)=>({
                    id: `fb-${Buffer.from(text || '').toString('base64').substring(0, 12)}-${index}`,
                    title: text ? text.substring(0, 90).replace(/\n/g, " ") + "..." : "Reporte Facebook",
                    summary: text || "",
                    source: "Organis Noticias",
                    publishedAt: new Date().toISOString(),
                    url: "https://www.facebook.com/organisnoticiasss",
                    neighborhood: "Córdoba" // El detector de barrios de abajo lo corregirá
                }));
            allArticles.push(...fbArticles);
            console.log(`✅ ${fbArticles.length} publicaciones de FB añadidas.`);
        }
    } catch (fbErr) {
        console.error("❌ Error recolectando Facebook:", fbErr);
    }
    // 3. Deduplicación y Detección de Barrios
    ...
    // 4. Clasificación con IA (Ahora clasifica RSS + FB juntos)
    if (deduped.length > 0) {
        try {
            console.log(`🧠 Clasificando ${deduped.length} noticias totales con DeepSeek...`);
    ...
    // 6. Generar SitRep (Incluirá el contexto de FB)
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0a8vdcu._.js.map