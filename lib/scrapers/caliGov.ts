...
const BASE_URL = "https://www.cordoba.gov.co";
const NEWS_URL = `${BASE_URL}/publicaciones/noticias/`;
const SOURCE = "Gobernacion de Cordoba";
...
export async function scrapeAlcaldia(pages = 3): Promise<Article[]> {
  ...
  for (let page = 1; page <= pages; page++) {
    ...
    try {
      ...
      if (!res.ok) {
        console.warn(`[cordobaGov] page ${page} → HTTP ${res.status}`);
        break;
      }
      ...
      console.error(`[cordobaGov] page ${page} failed:`, (err as Error).message);
      break;
    }
  }
